...
double SIM_decoder_delay(int C1, int B, int A, int Ndwl, int Ndbl, int Nspd, int Ntwl, int Ntbl, int Ntspd, double *Tdecdrive,
                         double *Tdecoder1, double *Tdecoder2, double *outrisetime)
{
    ...
    *Tdecdrive = SIM_horowitz(nextinputtime,tf,PARM(VTHINV100x60),PARM(VTHNAND60x90),
    ...
    numstack =
        (int)(ceil((1.0/3.0)*logtwo( (double)((double)C1/(double)(B*A*Ndbl*Nspd)))));
...
double SIM_decoder_tag_delay(int C1, int B, int A, int Ndwl, int Ndbl, int Nspd, int Ntwl, int Ntbl, int Ntspd,
                             double *Tdecdrive, double *Tdecoder1, double *Tdecoder2, double *outrisetime)
{
    ...
    *Tdecdrive = SIM_horowitz(nextinputtime,tf,PARM(VTHINV100x60),PARM(VTHNAND60x90),
    ...
    numstack =
        (int)(ceil((1.0/3.0)*logtwo( (double)((double)C1/(double)(B*A*Ntbl*Ntspd)))));
    if (numstack==0) numstack = 1;
    if (numstack>5) numstack = 5;
...
double SIM_wordline_tag_delay(int C1, int A, int Ntspd, int Ntwl, double inrisetime, double *outrisetime)
{
    double tf,m,a,b,c;
    ...
    tagbits = PARM(ADDRESS_BITS)+2-(int)logtwo((double)C1)+(int)logtwo((double)A);
...
double SIM_bitline_delay(int C1, int A, int B, int Ndwl, int Ndbl, int Nspd, double inrisetime, double *outrisetime)
{
...
double SIM_bitline_tag_delay(int C1, int A, int B, int Ntwl, int Ntbl, int Ntspd, double inrisetime, double *outrisetime)
{
    double Tbit,Cline,Ccolmux,Rlineb,r1,r2,c1,c2,a,b,c;
    double m,tstep;
    double Cbitrow;    /* bitline capacitance due to access transistor */
...
double SIM_compare_time(int C1, int A, int Ntbl, int Ntspd, double inputtime, double *outputtime)
{
    ...
    tagbits = PARM(ADDRESS_BITS) - (int)logtwo((double)C1) + (int)logtwo((double)A);
...
double SIM_mux_driver_delay(int C1, int B, int A, int Ndbl, int Nspd, int Ndwl, int Ntbl, int Ntspd, double inputtime, double *outputtime)
{
...
double SIM_valid_driver_delay(int C1, int A, int Ntbl, int Ntspd, double inputtime)
{
    double Ceq,Tst1,tf;
...
double SIM_dataoutput_delay(int C1, int B, int A, int Ndbl, int Nspd, int Ndwl,
                            double inrisetime, double *outrisetime)
{
    ...
    aspectRatio = (2.0*C1)/(8.0*B*B*A*A*Ndbl*Ndbl*Nspd*Nspd);
