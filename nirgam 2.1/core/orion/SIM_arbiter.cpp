...
double SIM_arbiter_stat_energy(SIM_arbiter_t *arb, SIM_array_info_t *info, double n_req, int print_depth, int max_avg)
{
    ...
    case QUEUE_ARBITER:
        ...
        Eavg = SIM_array_stat_energy(info, &arb->queue, n_req,n_grant, next_depth, max_avg);
        //SIM_res_path(path, path_len);
        break;
